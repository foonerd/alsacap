...
DOCKERFILE="docker/Dockerfile.alsacap.$ARCH"
IMAGE_TAG="alsacap-build-$ARCH"
...
# Clean previous build staging area thoroughly (but preserve out/$ARCH/*.deb)
echo "[+] Cleaning build staging area..."
rm -rf build/$COMPONENT/source

# Clean autotools-generated files and intermediates (if left from previous runs)
make clean || true
...
# Clean Debian intermediate and staging files
...
# Recreate build and output staging directory
mkdir -p build/$COMPONENT/source
mkdir -p out/$ARCH

# Copy project files to isolated build area
cp -a src/* build/$COMPONENT/source/
cp -a src build/$COMPONENT/source/
cp configure.ac build/$COMPONENT/source/
cp -r debian build/$COMPONENT/source/
cp Makefile.am AUTHORS ChangeLog NEWS README COPYING build/$COMPONENT/source/
cp -r man build/$COMPONENT/source/
